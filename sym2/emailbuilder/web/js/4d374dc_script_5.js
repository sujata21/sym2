...
function checkpoint(email_title){
            ...
            $.post("/app_dev.php/email/version/"+email_title, function(data) {
                ...
                $.each(res, function(index, val) {
                    ...
                    if(index != 0)
                      ver_str = ver_str+'<li class="list-group-item">Version : '+val+' <a style="padding-left:3px;cursor:pointer"onclick="getVersion(\''+email_title+'\',\''+verName+'\')" >view</a></li>';
            ...
            function getVersion(var1,verName){
               ...
               // verName_cnt = verName.length; 
                ...
                $("div#tabs").append('<div id="'+tabId+'"><div class="row" style="color:#588AA3;padding-left:31px;padding-bottom:12px;"><a onclick="restore(\''+var1+'\',\''+verLink+'\')" id="restore-link"><button type="button" class="btn btn-default">Restore</button></a></div><iframe src="/app_dev.php/email/viewVersion/'+var1+'/'+verLink+'" id="iframe'+verLink+'" width="1230" height="1820" name="alchemy" scrolling="no" frameborder="0"></iframe></div>');
                $("div#tabs").tabs("refresh");
