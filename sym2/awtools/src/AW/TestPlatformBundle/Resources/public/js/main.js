...
$(document).ready(function() {
	...
	//open IFrame Link Dialog
	$("#diviFrameLinkDialog").dialog({ autoOpen: false });
	$('#openiFrameLinkDialog').click(function() {
		$("#diviFrameLinkDialog").dialog({
			width: 520,
			resizable: false,
			maxHeight:600,
			//close: $('#formComments').trigger( "reset" ),
			buttons: {
				OK: function() {
					$('#spIframeLink').html($('#newIframeLink').val());
					$('#Iframe').attr('src', $('#newIframeLink').val());
					$(this).dialog( "close" );
				},
				Close: function() {
					$(this).dialog( "close" );
			...
			modal: true,});
		//$("#divAdminSavedComments").dialog('option', 'position', ['middle',100]);
		$('#diviFrameLinkDialog').dialog('open');
		$('#newIframeLink').val($('#Iframe').attr('src'));
	});
